-- Adiciona o campo "casa de oração" em alimentos, voluntários e solicitações pendentes
-- Execute este script no Supabase → SQL Editor (ou via supabase db push)

ALTER TABLE public.alimentos
  ADD COLUMN IF NOT EXISTS casa_oracao text NOT NULL DEFAULT '';
...
-- A função de aprovação precisa copiar casa_oracao do pendente para o voluntário final
...
begin
  ...
  insert into public.voluntarios (nome, telefone, disponibilidade, eh_lider, codigo, casa_oracao)
  values (v_pendente.nome, v_pendente.telefone, v_pendente.disponibilidade, false, upper(trim(p_codigo)), v_pendente.casa_oracao)
