...
import { Check, Copy, Crown, Pencil, Plus, Trash2, UserX, X } from "lucide-react";
...
export default function VoluntariosPage() {
  ...
  const remove = async (id: string) => {
    try { await excluirVoluntario(id); await loadPending(); toast.success("Voluntário movido para \"Em análise\"."); } catch (error) { toast.error(errorMessage(error)); }
  };
    ...
    <div className="space-y-6 pb-12">
      ...
      <Tabs defaultValue="lista">
        ...
        <TabsContent value="lista" className="space-y-4 mt-4">
          ...
          <div className="grid gap-3">{voluntarios.map((voluntario) => <Card key={voluntario.id}>
            <CardContent className="p-4"><div className="flex items-start justify-between gap-3">
          <div>
          <div className="flex items-center gap-2"><p className="font-semibold">{voluntario.nome}</p>{voluntario.idade != null && 
          <Badge variant="outline">{voluntario.idade} anos</Badge>}{voluntario.ehLider && <Badge className="gap-1"><Crown className="h-3 w-3" />Líder</Badge>}{voluntario.podeControlarAlimentos && <Badge variant="secondary">Alimentos</Badge>}{voluntario.possuiCarro && <Badge variant="secondary">Carro</Badge>}</div><p className="text-sm text-muted-foreground">{voluntario.telefone || "Sem telefone"} · {voluntario.disponibilidade || "Sem disponibilidade informada"}</p><p className="text-sm text-muted-foreground">{voluntario.casaOracao}</p><button className="mt-2 font-mono text-sm bg-muted px-2 py-1 rounded inline-flex items-center gap-2" onClick={() => void copyCode(voluntario.codigo)}>{voluntario.codigo}<Copy className="h-3 w-3" /></button></div><div className="flex"><Button variant="ghost" size="icon" onClick={() => setEditing(voluntario)}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" title="Mover para Em análise" onClick={() => void remove(voluntario.id)}><UserX className="h-4 w-4 text-destructive" /></Button></div></div></CardContent></Card>)}</div>
        </TabsContent>
